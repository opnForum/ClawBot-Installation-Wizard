...
select_main_menu() {
  ...
  echo -e "  ${CLR_DIM}│${CLR_RESET}  1) Install OpenClaw       4) Model Manager          ${CLR_DIM}│${CLR_RESET}" >&2
  echo -e "  ${CLR_DIM}│${CLR_RESET}  2) Troubleshoot           5) Uninstall OpenClaw     ${CLR_DIM}│${CLR_RESET}" >&2
  echo -e "  ${CLR_DIM}│${CLR_RESET}  3) Device Pairing         6) Exit                   ${CLR_DIM}│${CLR_RESET}" >&2
  echo -e "  ${CLR_DIM}└──────────────────────────────────────────────────────┘${CLR_RESET}" >&2
  echo >&2
...
wizard_install_questions() {
  ...
  if [[ "${MODE}" == "local" || "${MODE}" == "hybrid" ]]; then
    ...
    if ollama_present; then
      ...
      if ask_yes_no "Update Ollama to latest version?" "n"; then
        log "Updating Ollama..."
        curl -fsSL https://ollama.com/install.sh | sh
        sudo systemctl restart ollama
        ok "Ollama updated."
  ...
  if [[ "${MODE}" == "cloud" || "${MODE}" == "hybrid" ]]; then
    ...
    echo
    while true; do
      CLOUD_API_KEY="$(ask_input "Enter your ${CLOUD_PROVIDER} API key")"
...
write_compose_file() {
  ...
  if [[ "${MODE}" == "cloud" || "${MODE}" == "hybrid" ]]; then
    extra_env="${extra_env}
      - ${CLOUD_API_ENV_NAME}=${CLOUD_API_KEY}"
  fi
  if [[ "${MODE}" == "local" ]]; then
    # OpenClaw requires at least one recognized API key env var to start.
    # This dummy value satisfies the check but is never used since
    # local-only mode has no cloud fallback configured.
    extra_env="${extra_env}
      - OPENAI_API_KEY=unused-local-only"
  fi
...
install_flow() {
  ...
  if [[ -d "${PROJECT_DIR}" || -d "${CONFIG_DIR}" ]]; then
    warn "Existing OpenClaw files found at ~/openclaw or ~/.openclaw."
    warn "Continuing will overwrite your current installation."
    echo
  ...
  echo
  echo "Token URL:"
  echo "  https://${SERVER_IP}:${OPENCLAW_PORT}/#token=${GATEWAY_TOKEN}"
  echo
...
model_manager_flow() {
  ...
  current_primary="$(grep -o '"primary": *"[^"]*"' "${CONFIG_FILE}" | head -1 | sed 's/.*"primary": *"//' | sed 's/"//' || true)"
  local current_fallback
  current_fallback="$(grep -o '"fallbacks": *\["[^"]*"\]' "${CONFIG_FILE}" | head -1 | sed 's/.*\["//' | sed 's/"\]//' || true)"

  # Detect current cloud provider name from config
  local current_cloud_provider=""
  local has_ollama_provider has_cloud_provider
  has_ollama_provider="$(grep -c '"ollama"' "${CONFIG_FILE}" || true)"

  # Check for known cloud providers in the config
  for p in openai anthropic; do
    if grep -q "\"${p}\"" "${CONFIG_FILE}" 2>/dev/null; then
      # Make sure it's a provider entry, not just a model reference
      if grep -A2 "\"${p}\"" "${CONFIG_FILE}" | grep -q '"api"'; then
  ...
  GATEWAY_TOKEN="$(grep -o '"token": *"[^"]*"' "${CONFIG_FILE}" | head -1 | sed 's/.*"token": *"//' | sed 's/"//' || true)"
  SERVER_IP="$(grep -oP '"allowedOrigins":\s*\["https://\K[^:]+' "${CONFIG_FILE}" | head -1 || true)"
  OPENCLAW_PORT="$(grep -oP '"allowedOrigins":\s*\["https://[^:]+:\K[0-9]+' "${CONFIG_FILE}" | head -1 || true)"
  ...
  case "${choice}" in
    1)
      ...
      old_primary_line="$(grep '"primary":' "${CONFIG_FILE}")"
    ...
    2)
      ...
      local old_base_url
      old_base_url="$(grep -o '"baseUrl": *"[^"]*"' "${CONFIG_FILE}" | grep -v "11434" | head -1 | sed 's/.*"baseUrl": *"//' | sed 's/"//' || true)"
      ...
      # Update compose file and full restart only if provider changed
      if [[ "${provider_changed}" == "yes" ]]; then
        sudo sed -i '/_API_KEY=/{/OLLAMA_API_KEY/!d}' "${COMPOSE_FILE}"
        sudo sed -i "/OPENCLAW_NO_RESPAWN/a\\      - ${CLOUD_API_ENV_NAME}=${CLOUD_API_KEY}" "${COMPOSE_FILE}"
    ...
    3)
      ...
      # Update compose file only if provider changed
      if [[ "${provider_changed}" == "yes" ]]; then
        sudo sed -i '/_API_KEY=/{/OLLAMA_API_KEY/!d}' "${COMPOSE_FILE}"
        sudo sed -i "/OPENCLAW_NO_RESPAWN/a\\      - ${CLOUD_API_ENV_NAME}=${CLOUD_API_KEY}" "${COMPOSE_FILE}"
        sudo chmod 600 "${COMPOSE_FILE}"
